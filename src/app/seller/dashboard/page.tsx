...
export default function SellerDashboardPage() {
  ...
  async function handleDelete(id: number) {
    ...
    await fetch(`/api/sellers/products?id=${id}`, { method: "DELETE" });
    loadDashboard();
  }
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12" dir="rtl">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#2e2a24]">
            مرحباً، {seller?.name}
          </h1>
          {seller?.storeName && (
            <p className="text-sm text-[#2e2a24]/60">{seller.storeName}</p>
          )}
        </div>
        <div className="flex gap-3">
          ...
          <button
            ...
            className="rounded-full bg-[#faf6f0] px-5 py-2 text-sm font-medium text-[#2e2a24] hover:bg-red-50"
          >
            تسجيل خروج
          </button>
        </div>
      ...
      {showForm && (
        ...
        >
          ...
          <div className="rounded-lg border border-dashed border-[#2e2a24]/30 p-4">
            ...
            />
            {uploading && (
              <p className="mt-2 text-sm text-[#8a9a5b]">
                جاري رفع الصورة...
              </p>
            )}
      ...
      <div className="space-y-3">
        {products.length === 0 ? (
          <p className="text-[#2e2a24]/60">لسه معندكش منتجات مضافة</p>
        ) : (
          products.map((p) => (
            <div
              key={p.id}
